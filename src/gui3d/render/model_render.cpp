...
{
    if(!objs)  return;
    if(options._method == tOptions::HOLD_OFF)  objs->clear();
    ...
    for(const auto& pose : vPose)
    {
        ...
        kf->setColor(castColor(options._color));
        kf->setScale(options._scale);
        objs->insert(kf);
        ...
        CText::Ptr kfName = CText::Create(format("%d", id));
        kfName->setPose(castPose(vPose[index++]));
        kfName->setName(format("KFText%d", id));
        kfName->setScale(options._scale);
        objs->insert(kfName);
    }
}
...
void ZoomAxis(COpenGLScene::Ptr theScene, float freq)
{
    CGridPlaneXY::Ptr grid = theScene->getByClass<CGridPlaneXY>();
    if(grid)  grid->setGridFrequency(freq);

    CAxis::Ptr axis = theScene->getByClass<CAxis>();
    if(axis)
    {
...
bool VisibleText(COpenGLScene::Ptr theScene, bool bShow)
{
    size_t id = 0;
    while (theScene->getByClass<CText>(id))
    {
        CText::Ptr TextObj = theScene->getByClass<CText>(id);
...
bool renderLines(GLScenePtr theScene, CSetOfLines::Ptr& obj, const Position3dV& vvPoints, const tOptions& options)
{
    ...
    if (!vvPoints.empty() && vvPoints.size() >= 2)
    {
        ...
        obj->setColor(castColor(options._color));
        obj->setLineWidth(options._scale);
...
bool renderPointCloud(GLScenePtr theScene, CPointCloudColoured::Ptr& obj, const Pose& Twc, const PointCloud& vPoints, const tOptions& options)
{
	...
	if(options._method == tOptions::HOLD_OFF)
	{
		obj->clear();
...
bool viewImage(GLScenePtr theScene, COpenGLViewport::Ptr& gl_view_Image,  const cv::Mat& im)
{
    ...
    gl_view_Image->setImageView(cim);
